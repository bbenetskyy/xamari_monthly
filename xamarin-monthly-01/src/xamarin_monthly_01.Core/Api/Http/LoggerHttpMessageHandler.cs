    ...
    public class LoggerHttpMessageHandler : DelegatingHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                st.Start();
                HttpResponseMessage res = await base.SendAsync(request, cancellationToken).ConfigureAwait(false);
                st.Stop();
        ...
        private static async Task AppendResponseLog(StringBuilder sb, HttpResponseMessage response)
        {
            ...
            if (response.Content != null)
            {
                try
                {
                    bool canLogContent = response.Content.Headers.TryGetValues("Content-Type", out IEnumerable<string> values)
                        && values.Any(c =>
